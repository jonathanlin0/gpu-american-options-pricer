...
The main speedup from GPU comes from backwards induction. So, I wanted to isolate the speedup of just pricing 1 option, but we change the number of steps. This also gets rid of the GPU parallelization from running multiple blocks simultaneously. With my current CPU and GPU setup, the breakpoint is at about 425 steps. For each step value, 10 experiments were run. The shaded areas around the lines are the standard deviation for each point. This Figure generated with [`scripts/graph_steps_1_option.py`](scripts/graph_steps_1_option.py).
