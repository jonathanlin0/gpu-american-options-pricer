...
// Note: a lot of the math functions use ::
// This makes the code look in the global namespace for those functions and properly use the 
// global CUDA math overloads
...
void price_kernel(
    ...
    */
    ...
    double* old_payoffs = shared_payoffs;
    double* new_payoffs = shared_payoffs + (steps + 1);

    double delta_t = static_cast<double>(option.maturity_years) / steps;
    double u = ::exp(static_cast<double>(option.vol_annualized) * ::sqrt(delta_t));
    double d = ::exp(-static_cast<double>(option.vol_annualized) * ::sqrt(delta_t));
    double r = option.risk_free_rate_annualized;
    double q = (::exp(r * delta_t) - d) / (u - d); // risk-neutral chance of stock going up

    // create initial payoffs
    for (int i = threadIdx.x; i < steps + 1; i += blockDim.x) {
        double new_price = ::pow(u, i) * ::pow(d, steps - i) * option.spot;
        if (option.option_type == OptionType::Call) {
            old_payoffs[i] = ::fmax(new_price - option.strike, 0.0);
        } else {
            old_payoffs[i] = ::fmax(option.strike - new_price, 0.0);
        }
    }
    __syncthreads();
    ...
    for (int t = steps - 1; t >= 0; t--) {
        for (int i = threadIdx.x; i <= t; i += blockDim.x) {
            double current_spot = option.spot * ::pow(u, i) * ::pow(d, t-i);

            // value if the option is exercised now
            double curr_exercise_value;
            if (option.option_type == OptionType::Call) {
                curr_exercise_value = ::fmax(current_spot - option.strike, 0.0);
            } else {
                curr_exercise_value = ::fmax(option.strike - current_spot, 0.0);
            }

            // EV of next step, discounted by risk free rate r
            double continuation_value = ::exp(-r * delta_t) * (q * old_payoffs[i+1] + (1-q) * old_payoffs[i]);
    ...
    if (threadIdx.x == 0) {
        prices[blockIdx.x] = static_cast<float>(old_payoffs[0]);
    }
...
vector<float> GpuPricer::price(
    ...
    // 2 * (steps + 1) * 8 bytes <= 48 KB = 49152 B
    // steps <= 3071
    ...
    if (steps > 3071) {
        throw std::invalid_argument("steps must be less than or equal to 3071");
    }
    ...
    size_t shared_mem_bytes = 2 * (steps + 1) * sizeof(double);
