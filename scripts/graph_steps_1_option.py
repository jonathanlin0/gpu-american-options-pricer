...
OUTPUT_PATH = Path("figs") / "steps_1_option_graph.png"
...
def load_timings_by_step() -> tuple[list[int], list[float], list[float], list[float], list[float]]:
    ...
    with DATA_PATH.open("r", encoding="utf-8") as data_file:
        for line in data_file:
            ...
            run = json.loads(line)
            if run["num_options"] != TARGET_NUM_OPTIONS:
                continue
    ...
    if not cpu_times_by_step:
        raise ValueError(f"No benchmark rows found for num_options={TARGET_NUM_OPTIONS}")
...
def plot_steps_graph() -> None:
    ...
    plt.title("CPU vs GPU Batch Time by Binomial Steps (num_options = 1)")
